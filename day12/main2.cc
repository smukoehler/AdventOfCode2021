...
bool CheckSmallCaves(const Path& path) {
    std::map<std::string, int> small_cave_count;
    for (auto& node : path) {
        if (!IsUpper(node)) {
            if (small_cave_count.find(node) == small_cave_count.end()) {
                small_cave_count.insert({node, 1});
            } else {
                ++small_cave_count[node];
            }
        }
    }

    for (auto& cave : small_cave_count) {
        if (cave.second == 2) {
            return false;
        }
    }
...
bool IsLegal(const Path& path, const std::string& child) {
    ...
    if (IsUpper(child)) {
        return true;
    } else if ((child.compare("start") == 0)) {
        return false;
    } else if ((path.back().compare("end") == 0) || (path.back().compare("deadend") == 0)) {
        return false;
    } else {
        // Check if all small caves have only been visited once.
        int this_cave_count = std::count(path.begin(), path.end(), child);
        if (this_cave_count == 0 || CheckSmallCaves(path)) {
            return true;
        }
    }
    return false;
